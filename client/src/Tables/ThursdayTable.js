...
import { clearThursday, createThursday, getThursday } from '../Redux/Actions/thursdayAction';
...
const WednesdayTable = () => {
    ...
    const thursdayList = useSelector((state) => state.thursday.thursdayList);
    ...
    const handleCreate = (id) => {
        Swal.fire({
            ...
            if (res.value) {
                ...
                dispatch(createThursday(id, inputForm));
                setUpdate(true);
                setToggle(null);
            }
        }).catch((err) => {
    ...
    const renderTable = () => {
        return thursdayList.map((val,index) => {
            if (toggle === val.id) {
                return (
                    <tr key={index}>
                        <td>{val.id}</td>
