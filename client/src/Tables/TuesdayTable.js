...
import { clearTuesday, createTuesday, getTuesday } from '../Redux/Actions/tuesdayAction';
...
const TuesdayTable = () => {
    ...
    const tuesdayList = useSelector((state) => state.tuesday.tuesdayList);
    ...
    const handleCreate = (id) => {
        Swal.fire({
            ...
            if (res.value) {
                ...
                dispatch(createTuesday(id, inputForm));
                setUpdate(true);
                setToggle(null);
            }
        }).catch((err) => {
    ...
    const renderTable = () => {
        return tuesdayList.map((val,index) => {
            if (toggle === val.id) {
                return (
                    <tr key={index}>
                        <td>{val.id}</td>
