...
import { clearSaturday, createSaturday, getSaturday } from '../Redux/Actions/saturdayAction';
...
const WednesdayTable = () => {
    ...
    const saturdayList = useSelector((state) => state.saturday.saturdayList);
    ...
    const handleCreate = (id) => {
        Swal.fire({
            ...
            if (res.value) {
                ...
                dispatch(createSaturday(id, inputForm));
                setUpdate(true);
                setToggle(null);
            }
        }).catch((err) => {
    ...
    const renderTable = () => {
        return saturdayList.map((val,index) => {
            if (toggle === val.id) {
                return (
                    <tr key={index}>
                        <td>{val.id}</td>
