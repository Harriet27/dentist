...
import { clearSunday, createSunday, getSunday } from '../Redux/Actions/sundayAction';
...
const WednesdayTable = () => {
    ...
    const sundayList = useSelector((state) => state.sunday.sundayList);
    ...
    const handleCreate = (id) => {
        Swal.fire({
            ...
            if (res.value) {
                ...
                dispatch(createSunday(id, inputForm));
                setUpdate(true);
                setToggle(null);
            }
        }).catch((err) => {
    ...
    const renderTable = () => {
        return sundayList.map((val,index) => {
            if (toggle === val.id) {
                return (
                    <tr key={index}>
                        <td>{val.id}</td>
