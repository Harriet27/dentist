...
import { clearMonday, createMonday, getMonday } from '../Redux/Actions/mondayAction';
...
const MondayTable = () => {
    ...
    const mondayList = useSelector((state) => state.monday.mondayList);
    ...
    const handleCreate = (id) => {
        Swal.fire({
            ...
            if (res.value) {
                ...
                dispatch(createMonday(id, inputForm));
                setUpdate(true);
                setToggle(null);
            }
        }).catch((err) => {
    ...
    const renderTable = () => {
        return mondayList.map((val,index) => {
            if (toggle === val.id) {
                return (
                    <tr key={index}>
                        <td>{val.id}</td>
